...
class Octocat {
  ...
  colorCheck(svg) {
    ...
    if (this.options.hairColor) {
      ...
      for (let ai = 0; ai < hcc.length; ai++) {
        hcc[ai].setAttribute('fill', this.options.hairColor)
      }
    }

    if (this.options.facialHairColor) {
      let hcc = svg.querySelectorAll('.facialHair [fill="#9F592A"]')
      for (let ai = 0; ai < hcc.length; ai++) {
    ...
    if (this.options.prop && this.options.bodyColor) {
      ...
      let bc = svg.querySelectorAll('.props [fill="#282828"]')
  ...
  draw(p5, x, y, w, h) {
    ...
    p5.drawingContext.drawImage(this.image, x || 0, y || 0, w || 300, h || 300)
  }
